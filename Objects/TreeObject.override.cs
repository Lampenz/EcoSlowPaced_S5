...
namespace Eco.Mods.Organisms
{
    ...
    #pragma warning restore CS0067
		...
		private GameActionPack TrySliceTrunk(GameActionPack pack, INetObject damager, float amount, float slicePoint, Item tool)
		{
			...
			// Add post effect to either apply Perfect Cut talent or do normal slice
			pack.AddPostEffect(() => {
				if (!TryApplyPerfectCutTalent(damager, slicePoint))
				{
					// If we couldn't auto-split or the talent didn't trigger, do the normal slice
		...
		/// <summary>Shared method for trying to fully slice a trunk with the Perfect Cut talent</summary>
		private bool TryApplyPerfectCutTalent(INetObject damager, float slicePoint)
		{
			// Check for perfect cut talent
			var player = damager as Player;
			if (player != null && player.User != null)
			{
				bool hasPerfectCutTalent = player.User.Talentset.HasTalent(typeof(Eco.Mods.TechTree.LoggingPerfectCutTalent));
				if (hasPerfectCutTalent && RandomUtil.Value < 0.10f) // 10% chance
				{
					// Get the current trunk piece
					var trunk = this.GetTrunkPiece(slicePoint);
					if (trunk != null)
					{
						// Calculate trunk size and resource multiplier
						float trunkSize = trunk.SliceEnd - trunk.SliceStart;
						float resourcePerTrunk = trunkSize * this.ResourceMultiplier;
						
						// Calculate maximum number of possible slices
						int maxSlices = Mathf.FloorToInt(resourcePerTrunk);
						
						// Need at least 2 pieces to do any slicing
						if (maxSlices >= 2)
						{
							// Calculate equal piece size
							float pieceSize = trunkSize / maxSlices;
							
							var slicePoints = new List<float>();
							
							// Create slice points
							for (int i = 1; i < maxSlices; i++)
							{
								slicePoints.Add(trunk.SliceStart + (pieceSize * i));
							}
							
							// Sort slice points to ensure we slice from start to end
							slicePoints.Sort();
							
							// Apply all slices
							foreach (var point in slicePoints)
							{
								// Get the latest version of the trunk piece (it changes with each slice)
								var currentTrunk = this.GetTrunkPiece(point);
								if (currentTrunk != null)
								{
									this.TrySliceTrunkInternal(point, damager);
								}
							}
							
							// Notify the player that the perfect cut talent activated
							player.InfoBox(new LocString(typeof(Eco.Mods.TechTree.LoggingPerfectCutTalent).GetLocDisplayName().ToString() + " activated!"));
